...
class RmaMain(models.Model):
    ...
    def action_approve(self):
        ...
        if not self.delivery_order:
            ...
            # picking_type_id = self.env['stock.picking.type'].search([('code','=','incoming')])[0]
    ...
    #       'view_mode': 'tree,form', 
    #       'res_model': 'stock.picking', 
    #       'domain': [('rma_id','=',self.id),('picking_type_code','=','outgoing')], 
    #   }
    ...
    def create_delivery(self,value):
        ...
        stock_move_obj = self.env['stock.move']
        config_id = self.env['rma.config'].sudo().search([],order='id desc', limit=1)
        # picking_obj = self.env['stock.picking.type'].search([('code','=','outgoing')])[0]

        vals= {
            'rma_id' : self.id,
            'partner_id' : self.partner.id,

            'location_id' : config_id.b2b_destination_picking_type_id.default_location_src_id.id,
            'location_dest_id' : config_id.b2b_destination_picking_type_id.default_location_dest_id.id,
                
            'picking_type_code' : 'outgoing',
            'picking_type_id' : config_id.b2b_destination_picking_type_id.id,
            'state': 'done',
        }
    ...
    def create_credit_note_for_refundable_product(self,values):
        ...
        acc_invoice_obj = self.env['account.move']
        acc_inv_line_obj = self.env['account.move.line']
        product_obj = self.env['product.product']     
        rma_main = self.env['rma.main'].search([('id','=',values['rma_id'].id)])

        find_curr_prod = product_obj.search([('id','=',values['product_id'].id)])

        sale_line = self.env['sale.order.line'].search([('order_id','=',rma_main.sale_order.id),('product_id','=',find_curr_prod.id)],limit=1)
        ...
        invoice_for_rma = self.env['account.move'].search([('rma_id','=',rma_main.id)])
        if invoice_for_rma:
        ...
        acc_invoice.write({'invoice_line_ids': [(0,0,prepare_inv_line)] })
